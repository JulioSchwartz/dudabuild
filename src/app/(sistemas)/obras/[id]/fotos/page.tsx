...
export default function FotosObra() {
  ...
  async function uploadImagem(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (errUpload) {
      alert('Erro ao enviar imagem: ' + errUpload.message)
      setLoading(false)
      return
    }
    ...
    // Limpa o input para permitir envio da mesma foto novamente
    e.target.value = ''
    setLoading(false)
    carregar()
  }
  ...
  const fotosFiltradas = tipoAtivo === 'todos'
    ? fotos
    : fotos.filter(f => f.tipo === tipoAtivo)
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={filtroTipos}>
        <button
          ...
          style={tipoAtivo === 'todos' ? filtroAtivo : filtroInativo}
        >
          Todas ({fotos.length})
        </button>
        {TIPOS.map(t => {
