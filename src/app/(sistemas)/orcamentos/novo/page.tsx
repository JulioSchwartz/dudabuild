...
const itemMaterialVazio = () => ({ tipo: 'material', codigo: '', descricao: '', unidade: 'm²', quantidade: 1, material: 0, mao_obra: 0, equipamentos: 0 })
const itemServicoVazio  = () => ({ tipo: 'servico',  codigo: '', descricao: '', unidade: 'h',  quantidade: 1, material: 0, mao_obra: 0, equipamentos: 0 })
...
export default function NovoOrcamento() {
  ...
  function atualizarMaterial(index: number, campo: string, valor: any) { const novos = [...materiais]; novos[index][campo] = valor; setMateriais(novos) }
  function atualizarServico(index: number, campo: string, valor: any)  { const novos = [...servicos];  novos[index][campo] = valor; setServicos(novos) }
  ...
  function totalItem(i: any) { return (Number(i.material || 0) + Number(i.mao_obra || 0) + Number(i.equipamentos || 0)) * Number(i.quantidade || 0) }
  function totalGeral()      { return [...materiais, ...servicos].reduce((a, i) => a + totalItem(i), 0) }
  ...
  async function salvar() {
    ...
    try {
      ...
      const { data: orc, error } = await supabase.from('orcamentos').insert({
        empresa_id: empresaId, cliente_nome: cliente.trim(),
        telefone: telefone.replace(/\D/g, '') ? `+55${telefone.replace(/\D/g, '')}` : null,
        descricao: descricao.trim(), valor_total: totalGeral(), token,
      }).select().single()
      if (error || !orc) throw error || new Error('Orçamento não retornado')

      const itensPayload = [...materiais, ...servicos].map(i => ({
        orcamento_id: orc.id, codigo: i.codigo || null, descricao: i.descricao || null,
        unidade: i.unidade || null, quantidade: Number(i.quantidade || 1),
        material: Number(i.material || 0), mao_obra: Number(i.mao_obra || 0),
        equipamentos: Number(i.equipamentos || 0), valor_total: totalItem(i),
      }))
    ...
    } catch (err) { console.error('Erro ao salvar:', err); alert('Erro ao salvar orçamento. Tente novamente.') }
  ...
  function gerarPDF() {
    import('html2pdf.js').then(({ default: html2pdf }) => {
      ...
      el.innerHTML = `<div style="font-family:Arial;color:#0f172a">
        <div style="height:180px;background:#0f172a;color:white;display:flex;flex-direction:column;justify-content:center;align-items:center">
          <h1 style="margin:0;font-size:24px">Zynplan Engenharia</h1>
        ...
        <div style="padding:40px">
          ...
          ${materiais.map(i => `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:13px"><span>${i.codigo ? `[${i.codigo}] ` : ''}${i.descricao || '—'} (${i.quantidade} ${i.unidade})</span><strong>${format(totalItem(i))}</strong></div>`).join('')}
          ...
          ${servicos.map(i => `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:13px"><span>${i.codigo ? `[${i.codigo}] ` : ''}${i.descricao || '—'} (${i.quantidade} ${i.unidade})</span><strong>${format(totalItem(i))}</strong></div>`).join('')}
          ...
          <div style="margin-top:50px"><p>______________________________________</p><p><strong>Zynplan Engenharia</strong> · Responsável Técnico</p></div>
          <p style="margin-top:30px;text-align:center;font-size:11px;color:#94a3b8">Proposta válida por 7 dias</p>
        </div>
      </div>`
    ...
    <div style={container}>
      <style>{`
        @media (max-width: 768px) {
          .orc-form-grid { grid-template-columns: 1fr !important; }
      ...
      <div style={dadosCard}>
        ...
        <div className="orc-form-grid" style={formGrid}>
          <div style={formGrupo}>
          ...
          <div style={{ ...formGrupo, gridColumn: '1 / -1' }}>
            ...
            <input value={descricao} onChange={e => setDescricao(e.target.value)} placeholder="Ex: Construção de residência unifamiliar" style={input} />
          </div>
        </div>
      </div>
      ...
      <TabelaOrcamento
        ...
        atualizarMaterial={atualizarMaterial} atualizarServico={atualizarServico}
        removerMaterial={removerMaterial}     removerServico={removerServico}
      />
      ...
      <div className="orc-acoes" style={acoes}>
        <button onClick={salvar}    style={btnSalvar} disabled={loading}>{loading ? 'Salvando...' : '💾 Salvar Orçamento'}</button>
