...
export default function SistemaLayout({ children }: { children: React.ReactNode }) {
  ...
  async function verificar() {
    try {
      ...
      if (data.status !== 'active') { router.push('/bloqueado'); return }
      ...
      <aside style={sidebar}>
        ...
        <div>
          ...
          <nav>
            ...
            <MenuItem texto="🏢  Perfil da Empresa" rota="/perfil" ativo={pathname.startsWith('/perfil')} />
          </nav>
        </div>
...
const loadingSpinner: React.CSSProperties = { width: 36, height: 36, border: '3px solid #1e293b', borderTop: '3px solid #3b82f6', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }
const container: React.CSSProperties = { display: 'flex', height: '100vh', background: '#f1f5f9', overflow: 'hidden' }
const sidebar: React.CSSProperties = { width: 260, minWidth: 260, background: '#020617', color: '#fff', padding: '24px 16px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '4px 0 20px rgba(0,0,0,0.3)', zIndex: 10 }
const logoBox: React.CSSProperties = { marginBottom: 20, paddingLeft: 4 }
const logoText: React.CSSProperties = { marginTop: 8, fontSize: 13, fontWeight: 700, color: '#475569', letterSpacing: '0.08em', textTransform: 'uppercase' }
const divider: React.CSSProperties = { height: 1, background: '#1e293b', margin: '12px 0' }
const menuLabel: React.CSSProperties = { fontSize: 10, fontWeight: 700, color: '#475569', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '8px 14px 4px' }
const menuItem: React.CSSProperties = { padding: '11px 14px', borderRadius: 8, cursor: 'pointer', marginBottom: 4, fontSize: 14, fontWeight: 500, transition: 'background 0.15s, color 0.15s', display: 'flex', alignItems: 'center', gap: 8 }
const userInfo: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 4px', marginBottom: 10 }
const avatar: React.CSSProperties = { width: 36, height: 36, minWidth: 36, borderRadius: '50%', background: '#1d4ed8', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 15 }
...
const userCompany: React.CSSProperties = { fontSize: 11, color: '#475569', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }
const btnSair: React.CSSProperties = { width: '100%', background: '#1e293b', border: '1px solid #334155', color: '#94a3b8', padding: '10px 14px', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }
const mainWrapper: React.CSSProperties = { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }
const topbar: React.CSSProperties = { height: 64, background: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 28px', borderBottom: '1px solid #e2e8f0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', flexShrink: 0 }
const topbarTitle: React.CSSProperties = { fontSize: 16, fontWeight: 700, color: '#0f172a' }
const topbarSub: React.CSSProperties = { fontSize: 12, color: '#94a3b8', marginTop: 2 }
const btnUpgrade: React.CSSProperties = { background: 'linear-gradient(135deg, #2563eb, #7c3aed)', color: '#fff', border: 'none', padding: '8px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }
