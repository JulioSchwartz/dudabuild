...
export default function SistemaLayout({ children }: { children: React.ReactNode }) {
  ...
  async function verificar() {
    try {
      ...
      // Só bloqueia se cancelado ou inadimplente — incomplete é trial/novo
  ...
  return (
    <div style={container}>
      ...
      <aside style={sidebar}>
        ...
        <div>
          <div style={logoBox}>
            <img src="/logo.png" alt="DudaBuild" style={{ width: 100, display: 'block' }} />
          </div>
        ...
        <div>
          ...
          <p style={versao}>DudaBuild v1.0</p>
        </div>
...
function MenuItem({
  texto, label, rota, ativo, destaque = false
}: {
  ...
  return (
    <div
      ...
      style={{
        ...menuItem,
        background: ativo ? 'rgba(59,130,246,0.15)' : 'transparent',
        color:      ativo ? '#60a5fa' : destaque ? '#fbbf24' : '#94a3b8',
...
/* ── ESTILOS ── */
const loadingScreen: React.CSSProperties      = { height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f8fafc' }
const loadingSpinnerWrap: React.CSSProperties = { width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center' }
const loadingSpinner: React.CSSProperties     = { width: 32, height: 32, border: '3px solid #e2e8f0', borderTop: '3px solid #3b82f6', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }
...
const planoBadge = (plano: Plano): React.CSSProperties => ({
  ...
  background: plano === 'premium' ? 'rgba(234,179,8,0.15)' : plano === 'pro' ? 'rgba(139,92,246,0.15)' : 'rgba(59,130,246,0.12)',
  color:      plano === 'premium' ? '#fbbf24' : plano === 'pro' ? '#a78bfa' : '#60a5fa',
  border:     `1px solid ${plano === 'premium' ? 'rgba(234,179,8,0.3)' : plano === 'pro' ? 'rgba(139,92,246,0.3)' : 'rgba(59,130,246,0.25)'}`,
})
...
const avatar: React.CSSProperties            = { width: 34, height: 34, minWidth: 34, borderRadius: '50%', background: 'linear-gradient(135deg, #1d4ed8, #7c3aed)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14 }
...
const topbarAvatar: React.CSSProperties      = { width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg, #1d4ed8, #7c3aed)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13 }
const btnUpgrade: React.CSSProperties        = { background: 'linear-gradient(135deg, #2563eb, #7c3aed)', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'opacity 0.15s' }
