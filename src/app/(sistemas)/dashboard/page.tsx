...
export default function Dashboard() {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {lucro < 0 && <div style={alertaErro}>⚠️ Sua operação está no prejuízo</div>}
      {margem < 10 && margem > 0 && (
      ...
      <div style={{ marginTop: 30 }}>
        ...
        {listaObras.length === 0 && (
          <p style={{ color: '#94a3b8', padding: '12px 0' }}>Nenhuma movimentação ainda.</p>
        )}
...
function Card({ titulo, valor, cor, tipo }: any) {
  return (
    <div style={{ background: cor + '15', padding: 20, borderRadius: 12, border: `1px solid ${cor}`, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
      ...
      <h2 style={{ color: cor }}>
        {tipo === 'porcentagem' ? Number(valor).toFixed(2) + '%' : tipo === 'numero' ? valor : format(valor)}
      </h2>
    </div>
  )
}
...
const grid: React.CSSProperties        = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 16, marginBottom: 30 }
const graficoBox: React.CSSProperties  = { background: '#fff', padding: 20, borderRadius: 12, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }
const linhaHeader: React.CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', padding: '8px 10px', background: '#f8fafc', borderRadius: 8, marginBottom: 4 }
const linha: React.CSSProperties       = { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', padding: 10, borderBottom: '1px solid #e2e8f0' }
const alertaErro: React.CSSProperties  = { background: '#fee2e2', padding: 12, borderRadius: 8, marginBottom: 10, color: '#991b1b' }
const alertaAviso: React.CSSProperties = { background: '#fef3c7', padding: 12, borderRadius: 8, marginBottom: 10, color: '#92400e' }