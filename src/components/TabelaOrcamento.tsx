  ...
  return (
    <div style={{ marginTop: 20 }}>
      <style>{`
        ...
        .sinapi-dropdown { position: fixed; background: #fff; border: 1px solid #e2e8f0; border-radius: 10px; box-shadow: 0 8px 32px rgba(0,0,0,0.15); z-index: 99999; max-height: 280px; overflow-y: auto; }
...
function SecaoTabela({ titulo, cor, itens, tipo, atualizar, remover, readOnly }: any) {
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {/* Container SEM overflow:hidden para não cortar dropdown */}
...
function AutocompleteDescricao({ item, index, tipo, atualizar, readOnly, inputStyle }: any) {
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      ...
      if (!target.closest('.sinapi-dropdown') && inputRef.current !== target) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%' }}>
      <input
        ...
        style={{ ...inputStyle, width: '100%', paddingRight: buscando ? 28 : undefined }}
      />
      {buscando && (
      ...
      {mounted && mostrarLista && sugestoes.length > 0 && createPortal(
        <div className="sinapi-dropdown" style={{ top: dropdownPos.top, left: dropdownPos.left, width: Math.max(dropdownPos.width, 320) }}>
          {sugestoes.map(s => (
            <div key={s.codigo} className="sinapi-dropdown-item" onMouseDown={() => selecionarSinapi(s)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: '#0f172a', flex: 1, lineHeight: 1.3 }}>{s.descricao}</span>
                <span style={{ fontSize: 12, color: '#16a34a', fontWeight: 700, whiteSpace: 'nowrap' }}>{format(s.valor)}/{s.unidade}</span>
              </div>
...
function AutocompleteCodigo({ item, index, tipo, atualizar, readOnly, inputStyle }: any) {
  ...
  async function buscarPorCodigo(codigo: string) {
    ...
    const { data } = await supabase
      ...
      .single()
    if (data) {
      atualizar(index, 'codigo',    data.codigo)
      atualizar(index, 'descricao', data.descricao)
  ...
  function handleChange(valor: string) {
    atualizar(index, 'codigo', valor)
    clearTimeout(debounceRef.current)
    debounceRef.current = setTimeout(() => buscarPorCodigo(valor), 500)
  }
...
function LinhaDesktop({ item, index, tipo, atualizar, remover, readOnly, corBorda }: any) {
  function totalLinha() {
    ...
    if (tipo === 'material') return valorComBdi(Number(item.material || 0), bdi) * qtd
    return valorComBdi(Number(item.mao_obra || 0) + Number(item.equipamentos || 0), bdi) * qtd
  }
  ...
  return (
    <div style={{
      ...
      alignItems: 'center', borderLeft: `3px solid ${corBorda}30`
    }}>
      ...
      <div style={{ position: 'relative' }}>
        <input disabled={readOnly} type="number" value={item.bdi || ''}
          ...
          placeholder="0"
          style={{ ...inputSt, textAlign: 'center', borderColor: '#d1fae5', width: '100%', paddingRight: 20 }}
          min="0" step="0.5" />
    ...
    <div style={{ padding: '14px', borderBottom: '1px solid #f1f5f9', borderLeft: `3px solid ${corBorda}40`, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 75px 70px', gap: 8 }}>
        <div>
          ...
          <AutocompleteCodigo item={item} index={index} tipo={tipo} atualizar={atualizar} readOnly={readOnly} inputStyle={inp} />
        </div>
        <div>
      ...
      <div>
        ...
        <AutocompleteDescricao item={item} index={index} tipo={tipo} atualizar={atualizar} readOnly={readOnly} inputStyle={inp} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: tipo === 'servico' ? '1fr 1fr 80px' : '1fr 80px', gap: 8 }}>
        ...
        <div>
          ...
          <input disabled={readOnly} type="number" value={item.bdi || ''}
            onChange={e => atualizar(index, 'bdi', Number(e.target.value))}
            placeholder="0"
            style={{ ...inp, textAlign: 'center', borderColor: '#d1fae5' }} min="0" step="0.5" />
        </div>
      </div>
